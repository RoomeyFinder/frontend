...
const validateFormDataFields = (formData: {[x:string]: string | boolean}, fields: string[]) => {
  type formDataKey = keyof typeof formData
  return fields.every(field => formData[field as formDataKey])
}

const findErrorFields = (formData: {[x:string]: string | boolean}, fields: string[]) => {
  type formDataKey = keyof typeof formData
...
export default function SignupProvider({ children }: {
  ...
  const { profileInitials, contactDetails, emailVerificationDetails, locationDetails, handleFormDataChange } = useManageFormData()

  const sendEmailVerificationCode = useCallback(async (successCallback: () => void) => {
    setLoading(true)
  ...
  const resendVerificationEmail = useCallback(async () => {
    setLoading(true)
    const body: {[x:string]: string} = { email: contactDetails.formData.email }
    const res = await fetchData({
      url: "/users/verify-email", method: "post", body })
    if (res.statusCode === 200) toast({ description: res.message, status: "success" })
    else toast({ description: res.message, status: "success" })
    setLoading(false)
  }, [contactDetails.formData.email, fetchData, toast])

  const verifyEmail = useCallback(async (successCallback: () => void) => {
    setLoading(true)
    const res = await fetchData({
    ...
    setLoading(false)
  }, [emailVerificationDetails.formData.verificationToken, fetchData, toast, contactDetails.formData.email])
  ...
  const submitSignup = useCallback(async () => {
    ...
    const res = await fetchData({ url: `/users/verify-email/${emailVerificationDetails.formData.verificationToken}`, method: "put", body })
    ...
    setLoading(false)
  }, [locationDetails.formData, fetchData, emailVerificationDetails.formData.verificationToken, toast, router, contactDetails.formData.email])
  ...
  const handleSubmitButtonClick = useCallback(() => {
    if (totalStages.currentStage === 1) {
      ...
      else { verifyEmail(() => emailVerificationAndAddressFlow.goToNextStage())}
    }
  ...
  const signupContextValue = useMemo(() => ({
    ...
    handleFormDataChange: handleFormDataChange((name) => setError(prev => prev.filter(it => it !== name))),
    profileInitials,
    contactDetails,
  ...
  useEffect(() => {
    ...
    if (unverifiedEmail && totalStages.currentStage !== 2 && emailVerificationAndAddressFlow.currentStage !== 1){
      handleFormDataChange((name: string) => setError(prev => prev.filter(it => it !== name)))("CONTACT", "email", unverifiedEmail)
      totalStages.navigateToStage(2)
      profileAndContactFlow.navigateToStage(2)
      emailVerificationAndAddressFlow.navigateToStage(1)
    }
  }, [emailVerificationAndAddressFlow, handleFormDataChange, profileAndContactFlow, totalStages])
...
function useManageFormData(){
  const [profileInitials, setProfileInitials] = useState({
    ...
    validate: (formData: { [x: string]: string | boolean }) => {
      ...
      const firstCheck = validateFormDataFields(formData, mainRequiredFields)
      ...
      const passedIsStudentCheck = formData["isStudent" as formDataKey] === true && (formData["school" as formDataKey] as string).length > 0
      const formErrorsFields = findErrorFields(formData, mainRequiredFields)
      if (formData.isStudent === true) return [passedIsStudentCheck && firstCheck, formErrorsFields.concat(!passedIsStudentCheck ? "school" : [])]
      return [passedIsNotAStudentCheck && firstCheck, formErrorsFields.concat(!passedIsNotAStudentCheck ? "occupation" : [])]
    }
  })
  const [contactDetails, setContactDetails] = useState({
    ...
    validate: (formData: { [x: string]: string }) => {
      ...
      const formErrorsFields = findErrorFields(formData, mainRequiredFields)
      return [validateFormDataFields(formData, mainRequiredFields) && passedPasswordCheck, formErrorsFields.concat(!passedPasswordCheck ? ["password", "confirmPassword"] : [])]
    }
  })
  ...
  const [locationDetails, setLocationDetails] = useState({
    ...
    validate: (formData: { [x: string]: string }) => {
      ...
      const formErrorsFields = findErrorFields(formData, mainRequiredFields)
      return [validateFormDataFields(formData, mainRequiredFields), formErrorsFields]
    }
  })
  ...
  const handleFormDataChange = useCallback((resetError: (name: string) => void) => (stageName: string, name: string, value: string | number | boolean) => {
    resetError(name)
