...
import {
  generateYearsListBetweenYears,
  getCountOfDaysInMonthBasedOnYear,
...
export default function DobInput({
  ...
  const today = useMemo(
    () => new Date(Date.now() - FOURTEEN_YEARS_IN_MILLISECONDS),
    []
  )
  ...
  const dateOptions = useMemo(() => {
    const numberOfDays = getCountOfDaysInMonthBasedOnYear(
      monthOptions.indexOf(dob.month),
      Number(dob.year)
    )
  ...
  const handleSelect: ChangeEventHandler<HTMLSelectElement> = useCallback(
    (e) => {
      ...
      setDob({
        ...
        month: monthOptions[updatedDate.getMonth()],
      ...
      trigger={
        ...
        >
          <Input
            ...
            name="dob"
          />
          <InputRightAddon border="0px" color="gray.100">
            <DownChevron />
          </InputRightAddon>
        </InputGroup>
      }
