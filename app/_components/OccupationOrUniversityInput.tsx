...
export default function OccupationOrUniversityInput({
  isStudent, toggleIsStudent, inputName, inputValue, handleChange, errors, spacing, inputVariant, columns
}: {
  isStudent: boolean
  ...
  errors: string[]
  spacing?: ResponsiveValue<number | (string & Record<string, never>) | "-moz-initial" | "inherit" | "initial" | "revert" | "revert-layer" | "unset">
  inputVariant?: string
  ...
  return (
    <SimpleGrid alignItems="center" columns={columns || { base: 1, sm: 2 }} spacing={spacing || { base: "1.8rem", sm: "3rem" }}>
      <GridItem>
        <CustomRadioGroup
          containerProps={{ spacing: { base: "1rem", md: "3rem" }, flexWrap: "wrap", alignItems: "center" }}
          name="isStudent"
          selectedValue={isStudent ? options[0] : options[1]}
          onChange={(val: string) => {
            toggleIsStudent(val.toLowerCase() === options[0].toLocaleLowerCase())
          }}
          options={options} />
      </GridItem>
      <GridItem>
        <SearchableInput
          ...
          handleChange={
            (value: string) => handleChange(inputName, value)} />
      </GridItem>
    </SimpleGrid>
  )
}